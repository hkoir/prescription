...
function connectCallSocket() {
  ...
  callSocket.onmessage = (e) => {
    ...
    if (data.type === "incoming_call") {    
     ...
     if (data.type === "doctor.joined") {
        alert(`${data.doctor_name} has joined the call`);
         console.warn("🔇 Docor has joing , please join now:");
        // Or update UI: show "Doctor is ready" message, enable video call button, etc.
      }
  ...
  ["click", "touchstart"].forEach(event => {
    joinBtn.addEventListener(event, () => {
      if (callAudio) {
        callAudio.pause();
        callAudio.currentTime = 0;
        callAudio = null; // Reset the global variable
      }
    ...
    dismissBtn.addEventListener(event, () => {
    if (callAudio) {
        callAudio.pause();
        callAudio.currentTime = 0;
        callAudio = null; // Reset the global variable
